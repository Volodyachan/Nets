...
def mtu_detection():
    ...
    if not validate_host(target_host):
        print(f'Имя хоста {target_host} некорректно или не может быть разрешено')
        return 1
    ...
    while high - low > 1:
        ...
        if successful_ping == 0:
            print('Исключение MTU')
            return 1
        elif successful_ping == 1:
            low = midpoint
            if full_mode:
                print(f'Пинг {target_host} с размером {midpoint}, код: 0')
        else:
            high = midpoint
            if full_mode:
                print(f'Пинг {target_host} с размером {midpoint}, код: 1')

    print(f'MTU для хоста {target_host} = {low} байт, размер полного пакета = {low + 28} байт')
